...
<template>
  <el-card class="box-card props">
    ...
    <el-row>
      <el-button class="props-btn" type="primary" @click="onConfirm">{{ $t('shardingsphere.btn.submit') }}</el-button>
    </el-row>
  </el-card>
</template>
<script>
...
export default {
  ...
  methods: {
    getProps() {
      this.$$api_shardingsphere4_getProps({
        data: {},
        fn: json => {
          const res = json.data;
    ...
    onConfirm() {
      this.$$api_shardingsphere4_putProps({
