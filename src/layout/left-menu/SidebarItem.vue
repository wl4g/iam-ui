<template>
  <div v-if="!item.hidden">
    ...
    <el-submenu v-else ref="subMenu" :index="resolvePath(item.path)" popper-append-to-body>
      <template slot="title">
        <item :icon="item.icon" :title="getMenuName(item)" />
      </template>
      <sidebar-item
  ...
  methods: {
    hasOneShowingChild(children = [], parent) {
      ...
      if (showingChildren.length === 1) {
        return false//20200514modify: if true when just had one child,it just displayed child, if false ,it will show the parent too;
      }
