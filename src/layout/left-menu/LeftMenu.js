  ...
  methods: {
    ...
    setRouterGroup() {
      ...
      routerList.forEach(function (item) {
        if (!item.classify) {
          // Ignore no classify menu
          return
        }
        let classifyDict = self.dictutil.getDictByTypeAndValue(
          "menu_classify_type",
          item.classify
        )
        if (classifyDict && item.routePath) {
          let classifyData =
            result[classifyDict.value] || (result[classifyDict.value] = {})
          classifyData["items"] = classifyData["items"] || []
          // 使用if-elseif是为了区分只搜索当前语言下的关键字
          if (
            self.$i18n.locale === "zh_CN" &&
            item.nameZh.toUpperCase().includes(keyWords)
          ) {
            classifyData["items"].push(item)
          } else if (
            self.$i18n.locale === "en_US" &&
            item.nameEn.toUpperCase().includes(keyWords)
          ) {
            classifyData["items"].push(item)
          }
    ...
    getClassifyName(classifyData) {
      return this.$i18n.locale == "en_US"
        ? classifyData.classifyNameEn
        : classifyData.classifyNameZh
    },
    ...
    handleDocElmClick(e) {
      if (
        document.querySelector(".sidebar-lightbox-header").contains(e.target)
      ) {
        return
      }
    ...
    parentLevelMenuClick(e) {
      ...
      if (typeof e == "string" && e.constructor == String) {
        let [currentPage] = this.$store.state.router.routList.filter(
          item => item.path == e
        )
        ...
        let [currentPage] = this.$store.state.router.routList.filter(
          item => item.path == window.location.hash.replace("#", "")
        )
